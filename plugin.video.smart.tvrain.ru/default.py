...
def live(params):
	...
	if quality == 'Спрашивать':
		for i in Data['HLS_SMARTTV_TEST']:
			AddItem(i['label'].encode('UTF-8'), url={'mode':'PlayHLS', 'url':i['url']}, isFolder=False)
		xbmcplugin.endOfDirectory(plugin_handle)
	else:
		for i in Data['HLS_SMARTTV_TEST']:
			if i['label'].encode('UTF-8') == quality:
				PlayHLS({'url': urllib.quote(i['url'])})
				break
...
def search(params):
	...
	if not query:
		userInput = xbmcgui.Dialog().input('Enter search query')
	else:
		userInput = query
	if not userInput:return

	url_ = 'http://tvrain.ru/archive/?query={0}&page={1}'.format(userInput, page)

	Data = Get_url(url_, Headers)
	if not Data: return

	ret = common.parseDOM(Data, "a", {"class": "article-item__cover" }, "href")
	if len(ret) == 0:
		xbmcgui.Dialog().ok('Ошибка', 'Ничего не найдено')
		return

	preTitles = common.parseDOM(Data, "a", {"class": "article-item__cover" })
	titles = common.parseDOM(preTitles, "img", ret = "alt")
	preview = common.parseDOM(preTitles, "img", ret = "src")

	allPagination = common.parseDOM(Data, "div", {"class": "pagination"})
	allPagination_a = common.parseDOM(allPagination, "a")
	total_pages = len(allPagination_a) + 1 # +1 because of the current page
	current_page = int(page)

	for idx, href in enumerate(ret):
		utfTitle = common.replaceHTMLCodes(titles[idx]).encode('UTF-8')
		parts = string.split(href, '-')
		if len(parts) != 2:
			continue
		id = parts[1][:-1] # without the last character

		info= {'type':'Video', 'Title': utfTitle, 'plot': utfTitle}
		property = {'fanart_image': preview[idx]}
		AddItem(utfTitle, {'mode': 'play','id': id}, isFolder = False, img = preview[idx], info = info, property = property)
	if current_page < total_pages:
		title = 'Далее > {0} из {1}'.format(str(current_page+1), str(total_pages))
		AddItem(title, {'mode': 'search', 'query': userInput, 'page': current_page+1})
	xbmcplugin.endOfDirectory(plugin_handle)
