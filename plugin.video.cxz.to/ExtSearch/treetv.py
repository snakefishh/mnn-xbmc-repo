...
class Treetv(Plugin):
	...
	def Search(self, args):
		...
		headers = {
			'Referer': 'http://tree.tv/',
			'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:45.0) Gecko/20100101 Firefox/45.0',
			'X-Requested-With': 'XMLHttpRequest',
		}
		Data = Get_url(url, headers=headers, Post={'usersearch': search})
		...
		if items:
			...
			for item in items:
				...
				href = a['href']
				title = a.string
				AddFolder(title + quolity, 'External_Search',
						  {'plugin': self.__class__.__name__, 'command': 'Content', 'href': href}, img=img, ico=img)
			return True
	...
	def Content(self, args):
		...
		for aitem in aitems:
			...
			if ahead:
				...
				AddFolder(atitle, 'External_Search',
						  {'plugin': self.__class__.__name__, 'command': 'Content2', 'dfolder': data_folder})
	...
	def Content2(self, args):
		...
		for js in js_[dfolder]:
			...
			AddFolder(str(js), 'External_Search',
					  {'plugin': self.__class__.__name__, 'command': 'Content3', 'dfolder': dfolder, 'ql': str(js)})
		return True
	...
	def Content3(self, args):
		...
		for con in js_[dfolder][ql]:
			title = con.split('?')[0]
			title = title.split('/')[-1]
			AddItem(title, 'External_Search',
					{'plugin': self.__class__.__name__, 'command': 'Play', 'title': title, 'url': con})
