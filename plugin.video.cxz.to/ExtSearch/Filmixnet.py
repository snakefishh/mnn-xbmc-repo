...
class Filmixnet(Plugin):
	...
	def Search(self, args):
		...
		if result:
			...
			for res in result:
				AddFolder(res['title'], 'External_Search',
						  {'plugin': self.__class__.__name__, 'command': 'FilmixNet_content', 'href': res['href']},
						  img=res['img'], ico=res['ico'])
			return
		else:
			return False
	...
	def FilmixNet_content(self, args):
		...
		if len(cont) > 1:
			for ple in range(0, len(cont)):
				AddFolder('Плеер ' + str(ple + 1), 'External_Search',
						  {'plugin': self.__class__.__name__, 'command': 'FilmixNet_content2', 'le': ple})
			return True
		else:
	...
	def FilmixNet_content2(self, args):  # Сезон
		...
		if type(cont[int(args['le'])]) == dict:
			for i in cont[int(args['le'])]['playlist']:
				AddFolder(i['comment'], 'External_Search',
						  {'plugin': self.__class__.__name__, 'command': 'FilmixNet_content3', 'le': args['le'],
						   'le2': i['comment'].encode('UTF-8')})
			return True
		else:
	...
	def FilmixNet_content3(self, args):  # Серия
		...
		for i in cont[int(args['le'])]['playlist']:
			if i['comment'].encode('UTF-8') == le2:
				for j in i['playlist']:
					AddItem(j['comment'], 'External_Search',
							{'plugin': self.__class__.__name__, 'command': 'FilmixNet_play',
							 'title': j['comment'].encode('UTF-8'), 'url': j['file']})
				return True
	...
	def search_(self, s):
		headers = {'Accept': '*/*',
				   ...
				   'Referer': 'http://pro-filmix.net/index.php?do=search',
		...
		Post = {'change_search': '0', 'count_blocks': '0', 'story': s,
				...
				'imdb_ot': '0', 'imdb_do': '10', 'imdbk_ot': '0', 'imdbk_do': '10', 'sortr': 'none', 'sortd': 'all',
				'sort': 'asc'}

		Data = Get_url('http://pro-filmix.net/engine/ajax/search_new.php?do=search&mode=advanced', headers=headers,
					   Post=Post)
